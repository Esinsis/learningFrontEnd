...
export default class Header extends Component {
    ...
    search = () => {
        ...
        /*fetch(`api/search/users?q=${keyword}`).then(
            response => {
                //response.json() 是一个Promise对象
                console.log("connecting server succeed!", response.json());
    ...
    search_async = async () => {
        ...
        try {
            ...
            PubSub.publish("USER_INFO", {isLoading: false, users: data.items});
        }catch (e) {
            PubSub.publish("USER_INFO", {isLoading: false, err: e.message});
        }
    }
        ...
        return (
            <section className="jumbotron">
                ...
                <div>
                    ...
                    <button onClick={this.search_async}>搜索</button>
                </div>
            </section>
        )
    }
